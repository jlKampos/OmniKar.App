...
var I18N = (function () {
  ...
  var translations = {
    pt: {
      ...
      'guide.s1.tip3dText': 'O mapa Omni vem com <strong>edifícios 3D e estradas realçadas ativos por defeito</strong> no HeadUnit e no Tracking. No seletor de temas, o botão cubo alterna os 3D e o botão de estradas a realce — podes desligar ou ligar à vontade. Nas <strong>Definições</strong> ("Edifícios 3D") escolhes até que velocidade os edifícios ficam visíveis: <strong>Padrão</strong> (~68 km/h) ou <strong>Estendido</strong> (~78 km/h).',
      ...
      'guide.s5.headunit': '🚗 <strong>HeadUnit no carro:</strong> mantém este switch <strong>sempre ligado</strong> — é o headunit que regista os KM reais da viatura.',
      'guide.s5.phone': '📱 <strong>Telemóvel só para ver dados:</strong> mantém este switch <strong>desligado</strong> — o carro regista os KM e o telemóvel só os deve consultar. Se o deixares ligado, podes estar acidentalmente numa viagem noutra viatura e atualizar os KM dessa viatura sem querer.',
      'guide.s5.nophone': 'ℹ️ <strong>Sem HeadUnit?</strong> Nesse caso podes usar o telemóvel para atualizar os KM.',
      'guide.s6.title': 'Backups e sincronização cloud',
      'guide.s6.p1': 'Faz login com a tua conta Google nas <strong>Definições</strong> (separador <strong>Nuvem</strong>) para guardar e sincronizar os teus dados na <strong>pasta privada da app</strong> no teu Google Drive. Não há servidores externos — só tu tens acesso.',
      'guide.s6.private': '🔒 A pasta do backup (<code>appDataFolder</code>) é invisível no Google Drive normal — só a própria app OmniKar, com a tua conta, lhe acede.',
      'guide.s6.actions': 'Ações disponíveis (Definições → Nuvem)',
      'guide.s6.a1': '<strong>Backup to Cloud</strong> — envia uma cópia dos teus dados (viaturas, manutenções, combustível, GPS…) para a nuvem. <strong>Nunca destrói o que já está lá.</strong>',
      'guide.s6.a2': '<strong>Update from Cloud</strong> — traz para o telemóvel a cópia existente na nuvem (ex.: para restaurar depois de uma reinstalação ou num segundo dispositivo).',
      'guide.s6.a3': '<strong>Park here</strong> no HeadUnit — além de terminar a viagem, faz <strong>backup automático</strong> da viatura sempre que estacionas.',
      'guide.s6.plate': 'A matrícula como chave de sincronização',
      'guide.s6.p2': 'Cada viatura é identificada pela <strong>matrícula</strong> (campo obrigatório ao criar uma viatura nova). Isso permite ao OmniKar perceber que <strong>a mesma matrícula</strong> num telemóvel e num HeadUnit é <strong>a mesma viatura</strong>.',
      'guide.s6.p3': 'Quando fazes backup num dispositivo e depois "Update from Cloud" noutro, os dados da mesma matrícula <strong>aparecem na mesma viatura</strong> — sem duplicados.',
      'guide.s6.warn': '⚠️ <strong>Matrícula já existe na cloud:</strong> se registares uma viatura com uma matrícula que já existe no backup, o OmniKar avisa-te. No conflito, <strong>a versão da cloud é mantida</strong> (protege o histórico existente) e a nova fica só no telemóvel.',
      'guide.s6.tip': '💡 Edições do mesmo carro:',
      'guide.s6.tipText': 'Para a mesma viatura (mesma matrícula e ID), vale a <strong>versão mais recente</strong> — seja escrita no telemóvel ou no HeadUnit. Assim podes alternar entre dispositivos sem perder alterações.',
      'guide.s7.title': 'Fluxo rápido (resumo)',
      'guide.s7.step1': '<strong>Abre uma viatura</strong> — toca nela na lista principal (ou configura a viatura de arranque nas Definições)',
      'guide.s7.step2': '<strong>Cria agendamentos</strong> — define intervalos para manutenções periódicas e deixa os alertas trabalhar por ti',
      'guide.s7.step3': '<strong>Regista manutenções e reparações</strong> — a partir dos agendamentos (botão "Registar") ou manualmente',
      'guide.s7.step4': '<strong>Ativa o GPS</strong> — liga o switch "Atualizar KM via GPS" se quiseres quilometragem automática; o tracking no mapa funciona sempre',
      'guide.s7.step5': '<strong>Consulta o histórico</strong> — vê o mapa de viagens no Tracking, os custos nas Estatísticas, e gera relatórios PDF',
      'guide.s7.step6': '<strong>Sincroniza a nuvem</strong> — faz login com a conta Google e usa "Backup to Cloud" para nunca perderes os dados.',
    ...
    en: {
      ...
      'guide.s1.tip3dText': 'The Omni map comes with <strong>3D buildings and highlighted roads active by default</strong> on both the HeadUnit and Tracking. In the theme selector, the cube button toggles 3D and the roads button toggles highlights — turn them on or off as you like. In <strong>Settings</strong> ("3D buildings") you choose how long buildings stay visible: <strong>Standard</strong> (~68 km/h) or <strong>Extended</strong> (~78 km/h).',
      ...
      'guide.s5.headunit': '🚗 <strong>HeadUnit in the car:</strong> keep this switch <strong>always ON</strong> — the headunit records the vehicle\'s real mileage.',
      'guide.s5.phone': '📱 <strong>Phone only for viewing data:</strong> keep this switch <strong>OFF</strong> — the car records the mileage and the phone should only view it. If left ON, you could accidentally be on a trip in another vehicle and update that vehicle\'s mileage without meaning to.',
      'guide.s5.nophone': 'ℹ️ <strong>No HeadUnit?</strong> Then you can use the phone to update the mileage.',
      'guide.s6.title': 'Backups and cloud sync',
      'guide.s6.p1': 'Sign in with your Google account in <strong>Settings</strong> (the <strong>Cloud</strong> tab) to store and sync your data in the <strong>app\'s private folder</strong> in your Google Drive. No external servers — only you have access.',
      'guide.s6.private': '🔒 The backup folder (<code>appDataFolder</code>) is invisible in the normal Google Drive — only the OmniKar app itself, with your account, can access it.',
      'guide.s6.actions': 'Available actions (Settings → Cloud)',
      'guide.s6.a1': '<strong>Backup to Cloud</strong> — uploads a copy of your data (vehicles, maintenance, fuel, GPS…) to the cloud. <strong>It never deletes what is already there.</strong>',
      'guide.s6.a2': '<strong>Update from Cloud</strong> — pulls the cloud copy into your phone (e.g. to restore after a reinstall or on a second device).',
      'guide.s6.a3': '<strong>Park here</strong> on the HeadUnit — besides ending the trip, it makes an <strong>automatic backup</strong> of the vehicle every time you park.',
      'guide.s6.plate': 'The license plate as the sync key',
      'guide.s6.p2': 'Each vehicle is identified by its <strong>license plate</strong> (required when creating a new vehicle). This lets OmniKar recognise that <strong>the same plate</strong> on a phone and a HeadUnit is <strong>the same vehicle</strong>.',
      'guide.s6.p3': 'When you back up on one device and then "Update from Cloud" on another, the data for the same plate <strong>shows up on the same vehicle</strong> — no duplicates.',
      'guide.s6.warn': '⚠️ <strong>Plate already in the cloud:</strong> if you register a vehicle whose plate already exists in the backup, OmniKar warns you. On conflict, <strong>the cloud version is kept</strong> (protecting the existing history) and the new one stays only on the phone.',
      'guide.s6.tip': '💡 Edits to the same car:',
      'guide.s6.tipText': 'For the same vehicle (same plate and ID), the <strong>most recent version</strong> wins — whether written on the phone or the HeadUnit. That way you can switch between devices without losing changes.',
      'guide.s7.title': 'Quick flow (summary)',
      'guide.s7.step1': '<strong>Open a vehicle</strong> — tap it in the main list (or configure the startup vehicle in Settings)',
      'guide.s7.step2': '<strong>Create schedules</strong> — define intervals for periodic maintenance and let the alerts work for you',
      'guide.s7.step3': '<strong>Record maintenance and repairs</strong> — from schedules ("Register" button) or manually',
      'guide.s7.step4': '<strong>Enable GPS</strong> — toggle "Update KM via GPS" if you want automatic mileage; map tracking always works',
      'guide.s7.step5': '<strong>Review history</strong> — view trip map in Tracking, costs in Statistics, and generate PDF reports',
      'guide.s7.step6': '<strong>Sync the cloud</strong> — sign in with your Google account and use "Backup to Cloud" so you never lose your data.',
    ...
    es: {
      ...
      'guide.s1.tip3dText': 'El mapa Omni viene con <strong>edificios 3D y carreteras resaltadas activados por defecto</strong> tanto en el HeadUnit como en Tracking. En el selector de temas, el botón cubo alterna los 3D y el botón de carreteras los resalta — púedes activarlos o desactivarlos cuando quieras. En <strong>Ajustes</strong> ("Edificios 3D") eliges hasta qué velocidad permanecen visibles: <strong>Estándar</strong> (~68 km/h) o <strong>Extendido</strong> (~78 km/h).',
      ...
      'guide.s5.headunit': '🚗 <strong>HeadUnit en el coche:</strong> mantén este switch <strong>siempre activado</strong> — es el headunit el que registra los KM reales del vehículo.',
      'guide.s5.phone': '📱 <strong>Móvil solo para ver datos:</strong> mantén este switch <strong>desactivado</strong> — el coche registra los KM y el móvil solo debe consultarlos. Si lo dejas activado, podrías estar accidentalmente en un viaje en otro vehículo y actualizar los KM de ese vehículo sin querer.',
      'guide.s5.nophone': 'ℹ️ <strong>¿Sin HeadUnit?</strong> En ese caso puedes usar el móvil para actualizar los KM.',
      'guide.s6.title': 'Copias de seguridad y sincronización en la nube',
      'guide.s6.p1': 'Inicia sesión con tu cuenta de Google en <strong>Ajustes</strong> (pestaña <strong>Nube</strong>) para guardar y sincronizar tus datos en la <strong>carpeta privada de la app</strong> de tu Google Drive. Sin servidores externos — solo tú tienes acceso.',
      'guide.s6.private': '🔒 La carpeta del backup (<code>appDataFolder</code>) es invisible en el Google Drive normal — solo la propia app OmniKar, con tu cuenta, puede acceder a ella.',
      'guide.s6.actions': 'Acciones disponibles (Ajustes → Nube)',
      'guide.s6.a1': '<strong>Backup to Cloud</strong> — sube una copia de tus datos (vehículos, mantenimientos, combustible, GPS…) a la nube. <strong>Nunca borra lo que ya hay.</strong>',
      'guide.s6.a2': '<strong>Update from Cloud</strong> — trae al móvil la copia existente en la nube (p. ej. para restaurar tras una reinstalación o en un segundo dispositivo).',
      'guide.s6.a3': '<strong>Park here</strong> en el HeadUnit — además de terminar el viaje, hace un <strong>backup automático</strong> del vehículo cada vez que aparcas.',
      'guide.s6.plate': 'La matrícula como clave de sincronización',
      'guide.s6.p2': 'Cada vehículo se identifica por su <strong>matrícula</strong> (campo obligatorio al crear un vehículo nuevo). Así OmniKar entiende que <strong>la misma matrícula</strong> en un móvil y en un HeadUnit es <strong>el mismo vehículo</strong>.',
      'guide.s6.p3': 'Cuando haces backup en un dispositivo y luego "Update from Cloud" en otro, los datos de la misma matrícula <strong>aparecen en el mismo vehículo</strong> — sin duplicados.',
      'guide.s6.warn': '⚠️ <strong>Matrícula ya existe en la nube:</strong> si registras un vehículo con una matrícula que ya existe en el backup, OmniKar te avisa. En el conflicto, <strong>se mantiene la versión de la nube</strong> (protege el historial existente) y la nueva queda solo en el móvil.',
      'guide.s6.tip': '💡 Ediciones del mismo coche:',
      'guide.s6.tipText': 'Para el mismo vehículo (misma matrícula e ID), vale la <strong>versión más reciente</strong> — sea escrita en el móvil o en el HeadUnit. Así puedes alternar entre dispositivos sin perder cambios.',
      'guide.s7.title': 'Flujo rápido (resumen)',
      'guide.s7.step1': '<strong>Abre un vehículo</strong> — tócalo en la lista principal (o configura el vehículo de inicio en Ajustes)',
      'guide.s7.step2': '<strong>Crea agendamientos</strong> — define intervalos para mantenimientos periódicos y deja que las alertas trabajen por ti',
      'guide.s7.step3': '<strong>Registra mantenimientos y reparaciones</strong> — desde los agendamientos (botón "Registrar") o manualmente',
      'guide.s7.step4': '<strong>Activa el GPS</strong> — activa el switch "Actualizar KM vía GPS" si quieres kilometraje automático; el tracking en el mapa funciona siempre',
      'guide.s7.step5': '<strong>Consulta el historial</strong> — ve el mapa de viajes en Tracking, los costes en Estadísticas, y genera informes PDF',
      'guide.s7.step6': '<strong>Sincroniza la nube</strong> — inicia sesión con tu cuenta de Google y usa "Backup to Cloud" para no perder nunca tus datos.',
